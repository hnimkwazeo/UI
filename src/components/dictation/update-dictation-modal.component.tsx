...
import { updateDictationAPI } from 'services/dictation.service';
import { uploadFileAPI } from 'services/file.service';
import type { IUpdateDictationTopic, IDictationTopic } from 'types/dictation.type';
...
interface Props {
    ...
    onClose: () => void;
    onFinish: () => void;
    initialData: IDictationTopic | null;
}

const UpdateDictationModal = ({ open, onClose, onFinish, initialData }: Props) => {
    const [form] = Form.useForm<IUpdateDictationTopic>();

    const [api, contextHolder] = notification.useNotification();

    const openNotification = (pauseOnHover: boolean, desc: string, type: IconType = 'success') => () => {
        api.open({
            message: 'Update dictation topic',
            description: desc,
            showProgress: true,
            pauseOnHover,
            duration: 3,
    ...
    const handleFinish = async (values: IUpdateDictationTopic) => {
        ...
        const dataToSubmit: IUpdateDictationTopic = {
            ...values,
            id: initialData.id,
            categoryId: values.categoryId || initialData.category.id,
            sentences: sentencesWithOrder
        };

        try {
            await updateDictationAPI(dataToSubmit);
            openNotification(true, 'Dictation topic updated successfully!', 'success')();
            onFinish();
            return true;
        } catch (error) {
            openNotification(true, 'Failed to update dictation topic.', 'error')();
            return false;
        }
    };
                                                    ...
                                                    customRequest={async ({ file, onSuccess, onError }) => {
                                                        try {
                                                            ...
                                                            if (res && res.data) {
                                                                ...
                                                                sentences[field.name].audioUrl = res.data.fileUrl;
...
export default UpdateDictationModal;